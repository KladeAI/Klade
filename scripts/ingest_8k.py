#!/usr/bin/env python3
"""Task 6: SEC 8-K material events (last 12 months) → filings + filing_chunks tables."""
...
BATCH_SIZE = 50
CHUNK_SIZE = 2000
MAX_8K_PER_COMPANY = 10  # last 10 8-K filings
...
def supabase_post(table, rows, retries=2):
    ...
    for attempt in range(retries + 1):
        try:
            ...
            print(f"  Insert error: {e.code} {body[:150]}"); return []
...
def supabase_post_minimal(table, rows, retries=2):
    ...
    for attempt in range(retries + 1):
        try:
            ...
            print(f"  Insert error: {e.code} {body[:150]}"); return 0
        except Exception as e:
            if attempt < retries: time.sleep(2); continue
            return 0
...
def search_8k(cik, count=10):
    ...
    try:
        ...
        for i, form in enumerate(forms):
            if form == '8-K' and dates[i] >= '2025-03-01':  # last ~12 months
                results.append({
...
def chunk_text(text, chunk_size=CHUNK_SIZE):
    chunks = []; words = text.split(); current = []; current_len = 0
    for word in words:
...
def main():
    resume_file = os.path.expanduser('~/.openclaw/workspace/startup/klade/scripts/.8k_progress')
    done_tickers = set()
    ...
    for i, company in enumerate(companies):
        ...
        try:
            filings = search_8k(cik, count=MAX_8K_PER_COMPANY)
            
            for filing in filings:
                filing_row = {
                    'company_id': company_id, 'ticker': ticker, 'filing_type': '8-K',
                ...
                chunk_rows = [{
                    ...
                    'filing_type': '8-K', 'chunk_index': idx, 'section': '8-K Event',
                    'content': chunk[:10000], 'token_count': len(chunk.split()),
                } for idx, chunk in enumerate(chunks)]
                
                chunk_count = 0
                for j in range(0, len(chunk_rows), BATCH_SIZE):
                    inserted = supabase_post_minimal('filing_chunks', chunk_rows[j:j+BATCH_SIZE])
    ...
    print(f"\n{'='*60}")
    print(f"8-K INGESTION COMPLETE")
